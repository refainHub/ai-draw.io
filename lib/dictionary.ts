// Static Chinese dictionary for internal web tool
// This replaces the dynamic i18n system with a fixed Chinese locale
...
export const dictionary = {
    ...
    settings: {
        ...
        aiProviderDescription:
            "使用您自己的 API 密钥来绕过使用限制。您的密钥仅存储在浏览器本地，不会存储在服务器上。",
        provider: "提供商",
        modelId: "模型 ID",
        apiKey: "API 密钥",
        ...
        diagramValidationDescription:
            "使用视觉语言模型验证生成的图表。需要支持视觉的模型，如 GPT-5.2 或 Sonnet-4.5。",
        enabled: "已启用",
        disabled: "已禁用",
        customSystemMessage: "自定义系统消息",
        customSystemMessageDescription:
            "添加自定义指令，将附加到 AI 的系统提示末尾。",
    ...
    errors: {
        ...
        continuationRetryLimit:
            "已达到继续重试限制（{max}）。图表可能过于复杂。",
    ...
    templates: {
        ...
        createTitle: "创建模板",
        createDescription:
            "保存常用 prompt 以便重复使用。模板帮助您快速开始常用工作流。",
        promptLabel: "提示词",
    ...
    modelConfig: {
        ...
        customEndpoint: "自定义端点 URL",
        minimaxBaseUrlHint:
            "使用 /anthropic 端点为 Anthropic 兼容 API（推荐），或使用 /v1 端点为 OpenAI 兼容 API",
        models: "模型",
