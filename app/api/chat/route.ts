...
async function handleChatRequest(req: Request): Promise<Response> {
    ...
    // Find the last USER message for user input extraction
    ...
    // Read client AI overrides from headers
    ...
    if (selectedModelId?.startsWith("server:")) {
        ...
        console.log(
            `[Server Model Lookup] ID: ${selectedModelId}, Found: ${!!serverModel}`,
        )
        if (serverModel) {
            serverModelConfig = {
    ...
    console.log(`[Client Overrides] modelId: ${clientOverrides.modelId}`)

    // Get AI model with optional client overrides
    const { model, providerOptions, modelId } = getAIModel(clientOverrides)
...
"""`
    ...
    // OpenAI-compatible endpoints with custom base URLs (e.g. vLLM, LMStudio, Ollama)
    // typically use open-source model chat templates that reject multiple system messages.
    // Merge into single system message when a custom endpoint is configured.
