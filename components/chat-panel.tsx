...
const STORAGE_SESSION_ID_KEY = "refain-draw-session-id"

// sessionStorage keys
const SESSION_STORAGE_INPUT_KEY = "refain-draw-input"
...
export default function ChatPanel({
    ...
    useEffect(() => {
        // Migrate old sessionStorage key to new key
        const oldInputKey = "next-ai-draw-io-input"
        const oldInput = sessionStorage.getItem(oldInputKey)
        if (oldInput !== null) {
            sessionStorage.setItem(SESSION_STORAGE_INPUT_KEY, oldInput)
            sessionStorage.removeItem(oldInputKey)
        }
    ...
    // Generate a unique session ID (restore from localStorage if available)
    const [sessionId, setSessionId] = useState(() => {
        if (typeof window !== "undefined") {
            // Migrate old session-id key to new key
            const oldSessionKey = "next-ai-draw-io-session-id"
            const oldSession = localStorage.getItem(oldSessionKey)
            if (oldSession !== null) {
                localStorage.setItem(STORAGE_SESSION_ID_KEY, oldSession)
